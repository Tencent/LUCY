...
from vita.util import data_util_v4 as data_util
...
from vita.constants import AUDIO_PH, PAD_TOKEN, EMOTION_TOKENS, STATE_TOKENS 
...
class ModelArguments:
    ...
    codec_embeddings: Optional[str] = field(default=None)
...
def train():
    ...
    if "whisper" in model_args.audio_encoder:
        model.model.audio_mm_projector.requires_grad_(not model_args.freeze_audio_encoder_adapter)
    elif "audio-encoder-qwen2-7b-instruct" in model_args.audio_encoder.lower():
        # must be after freezing audio_encoder
        model.model.audio_encoder.adpter.requires_grad_(not model_args.freeze_audio_encoder_adapter)
    else:
        raise ValueError(f"{model_args.audio_encoder} not implemented")
    ...
    if model_args.codec_embeddings is not None:
        print("loading pretrained codec embeddings")
        codec_embeddings = torch.load(model_args.codec_embeddings).to(model.model.embed_tokens.weight.data)
        nemb, hidden_size = codec_embeddings.shape
        model.model.embed_tokens.weight.data[-nemb:] = codec_embeddings
