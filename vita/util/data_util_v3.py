...
from .data_util import load_label_offset, load_audio
from . import conversation as conversation_lib
from ..constants import IGNORE_INDEX, AUDIO_PH, PAD_TOKEN, EM_NEUTRAL
...
class DataArguments:
    ...
    eval_emotion: Optional[List[str]] = field(default_factory=list)
...
def load_emotion(emotion_path, inds, tot):
    emotion_list = []
    with open(emotion_path) as f:
        for l in f:
            emotion_list.append(l.strip())
    assert len(emotion_list) == tot, f"number of emotions {len(emotion_list)} != {tot}"
    emotion_list = [emotion_list[i] for i in inds]
    return emotion_list


def load_data(audio_in, text_in, text_out, codec_out, emotion, max_keep, min_keep, tolerance):
    ...
    if emotion is not None:
        emotion_list = load_emotion(emotion, ixs, tot)
        assert num_samples == len(emotion_list)

    return num_samples, audio_paths, lengths, starts, ends, textin, codec_offsets_list, textout_offsets_list, emotion_list

def load_single_turn_data(audio_ins, text_ins, text_outs, codec_outs, emotions, max_keep, min_keep, tolerance, max_num_samples=None):
    ...
    if emotions is None or len(emotions) == 0:
        emotions = [None] * len(audio_ins)
        assert len(emotions) == len(text_ins)
    data = []
    for audio_in, text_in, text_out, codec_out, emotion in zip(audio_ins, text_ins, text_outs, codec_outs, emotions):
        num_samples, audio_paths, lengths, starts, ends, textin, codec_offsets_list, textout_offsets_list, emotion_list = load_data(
            audio_in, text_in, text_out, codec_out, emotion, max_keep, min_keep, tolerance
        )
        ...
        emotion_list = emotion_list[:nsamples] if emotion_list is not None else None
...
class TA2TADataset(Dataset):
    def __init__(
        ...
        if split == "train":
            ...
            self.audio_in, self.text_in, self.codec_out, self.text_out, self.emotion = (
                data_args.audio_in, data_args.text_in, data_args.codec_out, data_args.text_out, data_args.emotion
            )
        elif split == "eval":
        ...
        single_turn_data = load_single_turn_data(
            self.audio_in, self.text_in, self.text_out, self.codec_out, self.emotion,
        ...
        self.tasks_with_audio_output = set(["AQAA", "TQAA", "TTS", "ASRA", "ASRAE", "RQACONVA"])
    ...
    def extract_source(self, 
        ...
        codec_out, codec_offsets_list, emotion_list,
        task, index
    ):
        ...
        emotion = self.get_emotion(emotion_list, index)
        ...
        if task == "ASR":
            ...
            if _use_template_random > 0.5: # template 1
                ...
                convs = [
                    ...
                    {"role": "assistant",  "content": text_or + textout_str, "emotion": emotion}
                ]
        elif task == "ASRA":
            text_sq, wavpath_sq, _       = random.choice(self.start_query)
            convs = [
                {"role": "system", "content": "You are an audio agent whose task is to transcribe the input audio into text."},
                {"role": "user", "wavpath": [wavpath_sq, audio_data]},
                {"role": "assistant",  "content": textout_str, "codec": codec, "emotion": emotion}
            ]
        elif task == "ASRE" or task == "ASRAE":
            convs = [
    ...
    def tokenize_conversation(self, conversations, task, system_prompt=None) -> Dict:
        ...
        for i, sentence in enumerate(conversations):
            ...
            emotion = sentence.get("emotion", EM_NEUTRAL)
    ...
    def get_emotion(self, emotion_list, index):
        if self.emotion is None or emotion_list is None:
            return EM_NEUTRAL
