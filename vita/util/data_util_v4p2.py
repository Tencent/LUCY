# same as data_util_v4 but use audio encoder of whale model
import os
import re
import json
import torch
...
class TA2TADataset(Dataset):
    def __init__(
        ...
        self.tasks_with_audio_input = set(["ASR", "AQA", "AQAA", "ASRA", "ASRAE", "ER", "ASRE"])
        ...
        self.tasks_with_emotion = set(["ASRAE", "ASRE", "RQACONV_EMO", "RQACONVA_EMO", "ER"])
    ...
    def extract_source(self, 
        ...
        if task == "ASR":
            ...
            convs = [
                {"role": "system", "content": "You are an audio agent whose task is to label the emotion of the input audio."},
                {"role": "user", "wavpath": audio_data},
    ...
    def extract_input_output(self, batch):
        ...
        shifted_ER    = torch.LongTensor([self.codec_layer_shift(self.ER, i)    for i in range(self.audio_num_codebook)])
        ...
        for item in batch:
            ...
            if codec is not None:
                ...
                if "ASR" in task:
                    ...
                    one_before_last_ans_position = torch.where(labels < 0)[0][-1]
                    input_codec[one_before_last_ans_position, :self.audio_num_codebook] = shifted_ER
    ...
    def collate_fn(self, batch):
        ...
        audios, audio_feature_lengths = None, None
        if audio_lengths.sum() > 0:
