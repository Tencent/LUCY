...
def load_concatenated_data(audio_ins, text_ins, text_outs, codec_outs, max_keep, min_keep, tolerance):
    ...
    for audio_in, text_in, text_out, codec_out in zip(audio_ins, text_ins, text_outs, codec_outs):
        ...
        ends_cat += ends
        textin_cat += textin
        textout_offsets_list_cat += textout_offsets_list
        if has_codec_out:
            codec_offsets_list_cat += codec_offsets_list
    return (
...
class TA2TDataset(Dataset):
    ...
    def __init__(
        ...
        super(TA2TDataset, self).__init__()
        if split == "train":
        ...
        self.audio_tasks = set(["ASR", "AQA"])
        self.text_tasks = set(["TTS", "TQA"])
    ...
    def __getitem__(self, index) -> Any:
        ...
        did = self.get_dataset_idx(ix)
        task = self.tasks[did]

        
        textin_str = self.textin[ix].replace("\\n", "\n")
        textout_str = self.get_textout(ix).replace("\\n", "\n")
        codec = self.get_codec(ix) if self.codec_out is not None else None
    ...
    def get_codec(self, index):
        did = self.get_dataset_idx(index)
        with open(self.codec_out[did]) as f:
    ...
    def get_textout(self, index):
        did = self.get_dataset_idx(index)
        with open(self.text_out[did], "rb") as f:
            offset_s, offset_e = self.textout_offsets_list[index]
            f.seek(offset_s)
        ...
        for item in batch:
            ...
            for i in range(self.audio_num_codebook):
                codec_by_layer_padded[i] = torch.LongTensor(
                    [self.codec_layer_shift(self.PAD_A, i)] * (text_length+1) 
                )
    ...
    def extract_input(self, batch):
        ...
        for item in batch:
            ...
            if task == "AQA" or task == "ASR": # collate audio only
                codec_by_layer_padded = torch.zeros(
            ...
            elif task == "TQA" or task == "TTS": # collate text only
                codec_by_layer_padded = torch.zeros(
    ...
    def collate_fn(self, batch):
        ...
        audio_lengths = torch.LongTensor([
            item["audio_length"] for item in batch if item["task"] in self.audio_tasks
        ])
        
        audios, use_audio_indices = None, None
        if len(audio_lengths) > 0:
            audios = torch.cat([item["audio"] for item in batch if item["task"] in self.audio_tasks]) # B' x 80 x 3000
        else:
            audios = batch[0]["audio"].new(0,*batch[0]["audio"].shape[1:])
        use_audio_indices = torch.LongTensor([
            i for i, task in enumerate(tasks) if task in self.audio_tasks
        ])
        ...
        for bi, (inp_leng, outp_lengs) in enumerate(zip(input_lengths, output_lengths)):
            ...
            for li, outp_leng in enumerate(outp_lengs):
                ...
                if (self.add_codec_target and layer_type == "audio") or layer_type == "text":
...
def make_data_module(text_tokenizer: transformers.PreTrainedTokenizer, audio_processor, data_args) -> Dict:
    ...
    train_dataset = TA2TDataset(text_tokenizer=text_tokenizer, audio_processor=audio_processor, data_args=data_args)
    data_collator = train_dataset.collate_fn
    eval_dataset = TA2TDataset(text_tokenizer=text_tokenizer, audio_processor=audio_processor, data_args=data_args, split="eval")
