...
class ASRTTSDataset(Dataset):
    ...
    def extract_output(self, batch):
        ...
        for item in batch:
            ...
            for i in range(self.audio_num_codebook):
                ...
                codec_by_layer_padded[i, num_audio_pad_right:-(num_audio_pad_left+1)] = codec_by_layer[i]
                codec_by_layer_padded[i, -num_audio_pad_left-1] = self.EOA
    ...
    def collate_fn(self, batch):
        ...
        for inp, inp_leng, outp, outp_leng in zip(inputs, input_lengths, outputs, output_lengths):
            input_ids.append(torch.cat([inp, outp], dim=-1).T) # T x 8
            lengs.append(inp_leng + outp_leng)
