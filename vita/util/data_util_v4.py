...
class TA2TADataset(Dataset):
    def __init__(
        ...
        self.tasks_with_audio_output = set(["AQAA", "TQAA", "TTS", "ASRA", "RQACONVA", "RQACONVA_NTRL", "RQACONVA_EMO"])
        self.tasks_with_random_input = set(["RQACONV", "RQACONVA", "RQACONV_EMO", "RQACONVA_EMO", "RQACONV_NTRL", "RQACONVA_NTRL"])
        self.tasks_with_emotion = set(["ASRE", "RQACONV_EMO", "RQACONVA_EMO"])
    ...
    def extract_input_output(self, batch):
        ...
        for item in batch:
            input_ids, labels, codec, audio_lengths, speaker = (
                item["input_ids"], item["labels"],
                item.get("codec", None), item.get("audio_lengths", []), item.get("speaker", "ANS_A")
            )
