...
class VITAQwen2ForCausalLM(Qwen2ForCausalLM, VITAMetaForCausalLM):
    ...
    def forward(
        ...
        if input_ids.shape[1] > max_input_length:
            ...
            old_sample_size = attention_mask.sum()
            ...
            if states is not None:
                state_attention_mask = state_attention_mask[:,:max_input_length]
            print(f"modify input_ids {input_ids.shape} to {inputs_embeds.shape}, {old_sample_size} to {attention_mask.sum()} tokens {attention_mask.sum(-1)} indices: {indices} dids {dids} idxs {idxs}")
