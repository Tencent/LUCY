...
class VITACausalLMOutputWithPast(CausalLMOutputWithPast):
    ...
    loss_phone: Optional[torch.Tensor] = None
...
class VITAQwen2ForCausalLM(Qwen2ForCausalLM, VITAMetaForCausalLM):
    ...
    def forward(
        ...
        loss, loss_text, loss_phone, loss_audios = None, None, None, None
        if output_labels_attention_mask is not None:
            logits_mask_text = output_logits_attention_mask[...,-2]
            labels_mask_text = output_labels_attention_mask[...,-2]
            logits_text = logits[logits_mask_text][...,:text_vocab_size_padded]
            labels_text = input_ids[...,-2][labels_mask_text]
            loss_text = self.compute_loss(logits_text, labels_text)

            logits_mask_phone = output_logits_attention_mask[...,-1]
            labels_mask_phone = output_labels_attention_mask[...,-1]
            text_audio_vocab_size_padded = text_vocab_size_padded + audio_num_codebook * audio_vocab_size_padded
            logits_phone = logits[logits_mask_phone][...,text_audio_vocab_size_padded:]
            labels_phone = self.phone_shift_reverse(input_ids[...,-1][labels_mask_phone])
            loss_phone = self.compute_loss(logits_phone, labels_phone)
            ...
            loss = loss_text + loss_phone + sum(loss_audios)
    ...
    def phone_shift_reverse(self, shifted_input_id):
        return shifted_input_id - self.config.text_vocab_size_padded - \
            self.config.audio_vocab_size_padded * self.config.mm_audio_num_codebook
